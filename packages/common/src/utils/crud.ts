...
>({ api, baseURL, multipart }: GenerateCrudParams) {
  ...
  return {
    list: (params?: Filter) => api.get<IPaginatedData<Model>>(baseURL, { params }),
    show: (id: number | string) => api.get<IApiWrapper<Model>>(`${baseURL}/${id}`),
    create: (params: Create) => api.post<IApiWrapper<Model>>(`${baseURL}`, params, { headers }),
...
>({ api, baseURL, multipart }: GenerateCrudParams) {
  ...
  return {
    list: (params?: Filter) => api.get<IApiWrapper<Model[]>>(baseURL, { params }),
    show: (id: number | string) => api.get<IApiWrapper<Model>>(`${baseURL}/${id}`),
    create: (params: Create) => api.post<IApiWrapper<Model>>(`${baseURL}`, params, { headers }),
    update: (id: number | string, params: Update) =>
      multipart
